...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  String? message;
  bool get minor {
    final d = DateTime.tryParse(dob.text);
  ...
  Future<void> submit() async {
    ...
    try {
      final r = await widget.service.register({
        'name': name.text.trim(),
        'email': email.text.trim(),
        'password': password.text,
        'password_confirmation': confirm.text,
        'date_of_birth': dob.text.trim(),
        if (minor) 'guardian_name': guardian.text.trim(),
        if (minor) 'guardian_relationship': 'Guardian',
        if (minor) 'guardian_phone': guardianPhone.text.trim(),
      });
      if (mounted) setState(() => message = r['message']?.toString());
    } catch (_) {
      if (mounted)
        setState(
          () => message =
        ...
        child: ListView(
          ...
          children: [
            ...
            TextFormField(
              ...
              validator: (v) =>
                  DateTime.tryParse(v ?? '') == null ? 'Use YYYY-MM-DD' : null,
            ),
            if (minor) ...[
              TextFormField(
                controller: guardian,
                ...
                validator: (v) => minor && (v == null || v.trim().isEmpty)
                    ? 'Required for teens'
                    : null,
              ),
              TextFormField(
            ...
            Semantics(
              button: true,
              label: 'Create Church of Uganda Youth Platform account',
              child: FilledButton(
